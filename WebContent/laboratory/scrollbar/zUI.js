 ...
 * }
 */
$.zUI.addWidget("zPanel",{
	defaults : {
			iWheelStep:16,
			sBoxClassName:"zUIzPanelScrollBox",
			sBarClassName:"zUIzPanelScrollBar"
	},
	__creator:function(ele){
		var jqThis = $(ele);
		...
		var opts = jqThis.data($.zUI.zPanel.sOptsName);
		//创建滚动框
	  ...
	  //事件对象
		...
		//记录添加事件
		jqThis.data($.zUI.zPanel.sEventName,oEvent);
		//跟随滚动函数
		...
		__destroyer:function(ele){
			var jqEle = $(ele);
			if(jqEle.data($.zUI.zPanel.sFlagName)){
				var opts = jqEle.data($.zUI.zPanel.sOptsName);
