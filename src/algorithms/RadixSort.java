...
public class RadixSort
{
    ...
    public static void countingRadixSort222( String [ ] arr, int stringLen )
    {
        final int BUCKETS = 256;
        int N = arr.length;
        ...
        for( int pos = stringLen - 1; pos >= 0; pos-- )
        {
            ...
            for( int i = 0; i < N; i++ )
            {
            	out[ count[ arr[ i ].charAt( pos ) ]++ ] = arr[ i ];
            }
            String [ ] tmp = arr;
            arr = out;
            out = tmp;
...
//        System.out.println( "Elapsed: " + ( end - start ) );
        ...
        String[] abc= {"ba","ha","bc","ab"};
        countingRadixSort222(abc,2);
        System.out.println("结果："+Arrays.toString(abc));
